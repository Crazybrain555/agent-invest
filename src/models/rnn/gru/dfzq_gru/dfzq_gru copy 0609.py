...
class DFZQGRU(nn.Module):
    ...
    def __init__(self, config: DFZQGRUConfig):
        ...
        self.bn1 = nn.BatchNorm1d(d_h, affine=False)
    ...
    def forward(self, x: torch.Tensor, h0: Optional[torch.Tensor] = None, monitor_gates: bool = False):
        ...
        res = self.simple_mlp(fv)               # [B, d_h]
        res = self.bn1(res)                     # lock in scale per-feature
        pred = res.mean(dim=1, keepdim=True)    # [B,1], scale now fixed
