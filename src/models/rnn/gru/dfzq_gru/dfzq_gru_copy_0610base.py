...
class SimpleResidualMLP(nn.Module):
    """Pre‑Norm + GEGLU + Residual MLP (可学习/固定缩放)"""

    def __init__(self, dim: int, expansion: int = 4, dropout: float = 0.1, scale: float = 0.3):
        super().__init__()
        hidden = dim * expansion
        self.ln = nn.LayerNorm(dim)
        self.proj = nn.Linear(dim, hidden * 2, bias=False)
        self.fc_out = nn.Linear(hidden, dim, bias=False)
        self.drop = nn.Dropout(dropout)
        # 固定缩放；如果想让网络学习，可改为 nn.Parameter(torch.tensor(scale))
...
class DFZQGRU(nn.Module):
    ...
    def __init__(self, config: DFZQGRUConfig):
        ...
        self.head_proj = nn.Sequential(
            ...
            nn.BatchNorm1d(d_h, affine=False),                 # 保留 affine，让网络学 γ/β
            # nn.ReLU(),
        )
        
        # bn1 to normalize after simple_mlp
        self.fc1 = nn.Linear(in_features=d_h*2, out_features=d_h)
        
        self.bn1 = nn.BatchNorm1d(d_h, affine=False, track_running_stats=True)
        
        # simple two-layer MLP mapping d_h → d_h
        
        
        # self.simple_mlp = nn.Sequential(
        #     nn.Linear(d_h, d_h, bias=False),
        #     nn.ReLU(),
        #     nn.Dropout(dropout),
        #     nn.Linear(d_h, d_h, bias=False),
        #     # nn.BatchNorm1d(d_h, affine=False),   # or LayerNorm(d_h, eps=1e-6)
        #     nn.ReLU(),                          # << 新增非线性
        #       )
        
        mlp_expansion = getattr(config, 'mlp_expansion', 4)
        mlp_scale = getattr(config, 'mlp_scale', 0.3)
        
        self.simple_mlp = SimpleResidualMLP(d_h, expansion=mlp_expansion,
                                            dropout=dropout, scale=mlp_scale)
    ...
    def forward(self, x: torch.Tensor, h0: Optional[torch.Tensor] = None, monitor_gates: bool = False):
        ...
        # fv = self.simple_mlp(fv)               # [B, d_h]
        # fv = self.fc1(combined)
        # fv= self.bn1(fv)
        pred = fv.mean(dim=1, keepdim=True)
        # fv = self.bn1(fv)                     # lock in scale per-feature
        # pred = fv.mean(dim=1, keepdim=True)    # [B,1], scale now fixed
