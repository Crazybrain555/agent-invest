# -*- coding: utf-8 -*-
"""
统一的数据加载器模块 - 迁移自 DFZQ_GRU
支持 TSViT 等多种模型的数据加载需求
"""
...
def get_dataloader(
    ...
    *,
    ...
    label_type: str | None = None,  # 🚀 新增：标签类型选择 ('regression' | 'classification')
    classification_label_name: str | None = None,  # 🚀 新增：分类标签列名
    return_original_values: bool = False,  # 🚀 新增：是否返回原始return值用于IC计算
    ok_keys_lag: int | None = None,  # 🚀 新增：允许指定 ok_keys lag
) -> DataLoader:
    ...
    # 🚀 新增：从配置中获取标签类型参数
    label_type = label_type if label_type is not None else config.get("label_type", "regression")
    classification_label_name = classification_label_name if classification_label_name is not None else config.get("classification_label_name", None)
    return_original_values = return_original_values if return_original_values is not None else config.get("return_original_values", False)
    ok_keys_lag = ok_keys_lag if ok_keys_lag is not None else config.get("ok_keys_lag", None)

    if label_type not in ("regression", "classification"):
        raise ValueError(f"Invalid label_type: {label_type}. Must be 'regression' or 'classification'.")

    if label_type == "classification":
        print(f"🎯 使用分类标签模式: {classification_label_name or '默认分类标签'}")
    else:
        print(f"📊 使用回归标签模式")
    ...
    dataset_config = {
        **config,
        ...
        "label_type": label_type,             # 🚀 新增：传递标签类型
        "classification_label_name": classification_label_name,  # 🚀 新增：传递分类标签列名
        "return_original_values": return_original_values,  # 🚀 新增：传递原始return值开关
    }
