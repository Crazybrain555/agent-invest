...
class TrainingMonitor:
    ...
    def _monitor_single_layer(self, model: nn.Module, layer_name: str, step: int):
        """监控单个层的四连指标：weight_std → activation_std → grad_std → grad_norm"""
    ...
    def monitor_basic_metrics(
        ...
        # Core指标
        self.monitor_core_metrics(loss, grad_norm, lr, scaler, step)
    ...
    def monitor_comprehensive(
        ...
        # 基础监控（每step都执行，轻量级）
        self.monitor_basic_metrics(loss, grad_norm, lr, predictions, labels, step, scaler)
