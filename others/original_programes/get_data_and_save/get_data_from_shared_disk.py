...
class Get_data_from_sshared_disk():
   ...
   #设一个函数，这个函数能输入因子名称，返回一个量化投资部门网盘里该因子打分的每日打分信息，格式是三列，分别是tdate，stk_code和因子名称
    def combine_data_from_disk(self,signal_name,range=None):
    ...
    def combine_theme_data_from_disk(self, signal_name, range=None,path='\\\\space\\alpha\\dyn_broad_zz800_cne5citics_size_beta\\subtheme'):
        # 如果不存在这个选定的因子signal_name，warning
