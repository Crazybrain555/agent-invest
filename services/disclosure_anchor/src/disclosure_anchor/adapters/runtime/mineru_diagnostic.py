...
from disclosure_anchor.adapters.parsers.mineru_medium.artifacts import (
    MinerUMediumArtifactReader, PinnedArtifactTree,
)
...
def run_diagnostic_pdf(
    *, input_pdf: Path, source_pdf_sha256: str, source_page_count: int,
    ...
    with httpx.Client(trust_env=False, follow_redirects=False, transport=transport) as client:
        def request(method: str, url: str) -> tuple[int, bytes]:
            with client.stream(method, url, timeout=checkpoint(),
                               headers={"Accept-Encoding": "identity"}) as response:
                status = response.status_code
                exact = read_response(response)
            checkpoint()
            return status, exact
