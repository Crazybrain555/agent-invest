...
`asset_uri` / `evidence_refs` + `source_ref` 的 `evidence_refs` + `tracked_company` 的
`effective_lookback_days` / `effective_sync_seconds` / `effective_process_classes`（不入库、
不进 public view，contract 导出与测试用 DERIVED 白名单处理）。`evidence_refs` 只暴露可请求
URI、digest、media type 与 byte size；不得暴露 artifact role 或文件路径。
...
`VALIDATION_ERROR`；evidence URI 的 digest 未被该 unit locator 引用时返回 `NOT_FOUND`，
已发布 manifest/文件缺失或 hash/size/media 漂移返回 `EVIDENCE_INTEGRITY_ERROR`；
superseded reject 只在 document detail 与 document units 声明的
参数上生效。
...
admin 不在导出契约面内）。读侧公开错误码全集仍为上方 6 个。
