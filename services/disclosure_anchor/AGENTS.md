...
1. Write only `disclosure_core` and `disclosure_ops`. Consumers read the applicable versioned
   `disclosure_public` view (`document_units_v1` is the sole current DB Unit contract), the current v1 Filing
   API, the change feed, or explicit source references; no cross-service private-table dependency.
