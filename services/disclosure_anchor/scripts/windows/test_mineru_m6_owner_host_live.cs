...
public static class M6LiveParent {
    ...
    sealed class Session : IDisposable {
        ...
        public Session(string name,int seconds,int grace,int bootstrapSeconds=0){
            ...
            Dictionary<string,object> cfg=O("contract_version","m6.owner-deployment.v2","run_id",RunId,"bootstrap_bind_seconds",bootstrapSeconds==0?Math.Min(120,seconds+grace):bootstrapSeconds,"run_root",runroot,"mode","service_diagnostic","owner_source_sha256",sourceHash,"expected_node_sha256",node,"gpu_uuid",gpu,"nvml_dll_sha256",nvml,"port",PortNumber,"resources",resources,"max_artifacts",256,"max_artifact_bytes",16777216,"maximum_lease_ticks",f,"propagation_reserve_ticks",f,"roles",roles);
        ...
        public void BuildSpecForBind(){
            ...
            Spec=J(spec);SpecHash=Hash(Spec);Check(!File.Exists(Path.Combine(RunDir,"spec.json")),"test must not preseed native spec");
            Check(FileHash(Path.Combine(RunDir,"anchor.json"))==AnchorHash,"local spec construction did not move T0/anchor");
        }
        ...
        public string Request(Dictionary<string,object> command){return J(O("contract_version","m6.owner-request.v2","run_id",RunId,"spec_sha256",SpecHash??Label("not-yet-bound"),"request_id","request-"+(++serial).ToString(CultureInfo.InvariantCulture),"command",command));}
        ...
        public void BindOpen(){
            Check(!File.Exists(Path.Combine(RunDir,"spec.json")),"no hidden spec upload before bind");
            Send(O("kind","bind","anchor_sha256",AnchorHash,"spec_utf8",Spec),"controller","ok");
            Check(Utf8.GetString(Read(Path.Combine(RunDir,"spec.json"),49152))==Spec,"owner persisted exact authenticated spec payload");
            Check(FileHash(Path.Combine(RunDir,"spec.json"))==SpecHash,"owner spec hash matches independent sender");
            string prefix=FileHash(Path.Combine(RunDir,"events.jsonl"));
            Send(O("kind","bind","anchor_sha256",AnchorHash,"spec_utf8",Spec),"controller","ok");
            Check(FileHash(Path.Combine(RunDir,"events.jsonl"))==prefix,"identical rebind does not create another journal event");
            Check(FileHash(Path.Combine(RunDir,"anchor.json"))==AnchorHash,"identical rebind keeps original anchor");
            Send(O("kind","open"),"controller","ok");
        }
