...
`provider_unit_locator.v9.search_targets` 选中的一个字符串叶子；历史 v1-v8 使用同一闭合 decoder。
两者都绝不递归发现字段，也绝不连接
...
- body 的唯一输入是当前 writer 的 `provider_unit_locator.v9.search_targets`（历史 v1-v8 同义）；每个 target 必须绑定 provider
