...
1. 多级标题的**数据**早已完整：`heading_path` 是每单元必填的 1-4 级数组（379/379 覆盖），
   mixed parts 另带 `local_heading` 相对路径——把它拆成 title_1..title_4 多列不增加任何
   信息，反而把"深度可变"硬编码成固定列。`title` 的语义 = 叶子显示名（heading_path[-1]
   或表名/议案名），单值是对的。
2. 用户真正缺的是**可检索形态**：heading_path 是 jsonb，只能整串精确匹配、没法关键词
   ...
   视图内派生、不入库、不进哈希；今天就能 `LIKE '%现金流量%'`，06R 投影对同一字段
   建 FTS（setweight B）即为正式检索通道。
